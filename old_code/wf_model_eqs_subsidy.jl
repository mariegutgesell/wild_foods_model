##Code to set up equations for wild food model - WITH SUBSIDY, no temporal forcing 
...
@with_kw mutable struct ModelPar2
    ...
    o = 0.7 ##o = omnivory preference, preference for either consumer or resource, starting with fixed omnivory preference (essentially passive case)
...
@with_kw mutable struct ModelPar4
    ##spatial scale of foraging 
    Sc = 1  ##Sc = spatial scale of consumer foraging
    Sh = 1  ##Sh = spatial scale of resources 
    Q = 0 ##for now starting with Q = 0 (Q = (Sc-Sh)/Sh) - #Q is the area of consumers current habitat where consumer percieves prey as well mixed; so 1-Q is area that requres consumers to switch patches in order to gain access to resources in that portion of the patch
...
##second form of parameters, where omnivory is done using Gutgesell et al., active omnivory 
@with_kw mutable struct ModelPar3 
    ##spatial scale of foraging 
    Sc = 1  ##Sc = spatial scale of consumer foraging
    Sh = 1  ##Sh = spatial scale of resources 
    Q = 0 ##for now starting with Q = 0 (Q = (Sc-Sh)/Sh) - #Q is the area of consumers current habitat where consumer percieves prey as well mixed; so 1-Q is area that requres consumers to switch patches in order to gain access to resources in that portion of the patch
    w = 1   ##w = habitat preference for patch i 
        ##do we need separate w for each C/R? wR1, wR2, wC1, wC2 ? 
    ...
    o_fixed = 0.1
    H = 0.8 ##H = preference for groceries (G)
...
##
##STRUCTURE 2: Plotting dynamics, equilibrium, eigenvalue analysis 
##Solve ODE 
##set initial condition
u0 = [0.6, 0.8, 0.45, 0.61, 0.2]
tspan = (0.0, 1000.0)
...
for x0 in guesses
    ...
    if result.f_converged
        x_star = result.zero
        # Check if it's a new equilibrium
        if all(norm(x_star .- eq) > tolerance for eq in equilibria_2)
            push!(equilibria_2, x_star)
        end
    end
end


max_eigenvalues_2 = Float64[]  # to store max real parts

println("Number of distinct equilibria found: ", length(equilibria_2))

for (i, eq) in enumerate(equilibria_2)
    J = ForwardDiff.jacobian(x -> wrapped_model_vec(x, p, P_fixed), eq)
    λ = eigvals(J)
    λ_real = real.(λ)
    λ_max = maximum(λ_real)
